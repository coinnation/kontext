// useCanister.ts - OPTIMIZED ERROR #310 PREVENTION VERSION
...
// Support both object and string syntax for backward compatibility
export function useCanister(optionsOrString?: UseCanisterOptions | string) {
  // Handle string syntax (e.g., useCanister('main'))
  const options: UseCanisterOptions = typeof optionsOrString === 'string' 
    ? { requireAuth: false } 
    : (optionsOrString || {});
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        if (!cancelled) {
          console.error('useCanister initialization failed:', err);
